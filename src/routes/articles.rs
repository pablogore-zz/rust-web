...
fn post_articles(auth: Auth, new_article: Json<NewArticle>, conn: db::Conn) -> Result<Json<Value>, Errors> {
...
/// return multiple articles, ordered by most recent first
#[get("/articles?<params>")]
fn get_articles_with_params(params: FindArticles, auth: Option<Auth>, conn: db::Conn) -> Json<Value> {
...
fn unfavorite_article(slug: String, auth: Auth, conn: db::Conn) -> Option<Json<Value>> {
  db::articles::unfavorite(&conn, &slug, auth.id).map(|article| Json(json!({ "article": article })))
}
...
fn put_articles(slug: String, article: Json<UpdateArticle>, auth: Auth, conn: db::Conn) -> Option<Json<Value>> {
  // TODO: check auth
  db::articles::update(&conn, &slug, auth.id, &article.article).map(|article| Json(json!({ "article": article })))
}
...
fn post_comment(slug: String, new_comment: Json<NewComment>, auth: Auth, conn: db::Conn) -> Result<Json<Value>, Errors> {
  let mut errors = Errors {
    errors: new_comment
      .comment
      .validate()
