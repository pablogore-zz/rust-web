...
pub fn create(conn: &PgConnection, author: i32, title: &str, description: &str, body: &str, tag_list: &Vec<String>) -> ArticleJson {
...
pub fn update(conn: &PgConnection, slug: &str, user_id: i32, data: &UpdateArticleData) -> Option<ArticleJson> {
...
pub fn delete(conn: &PgConnection, slug: &str, user_id: i32) {
  let result = diesel::delete(articles::table.filter(articles::slug.eq(slug).and(articles::author.eq(user_id)))).execute(conn);
