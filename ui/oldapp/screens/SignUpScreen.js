...
export default compose(
  ...
  withMutation(SIGN_UP_USER_MUTATION, async ({ navigation, username, email, password, retypedPassword, setError, setEmail, setPassword, setRetypedPassword, mutate }) => {
    if (username.length < 1) {
      return setError(strings.errs.username);
    }
    ...
    if (password !== retypedPassword) {
      return setError(strings.errs.retypePassword);
    }
    setError('');
    try {
      ...
      await storageUtils.set('token', res.data.signUpUser.token);
      setUsername('');
      setEmail('');
      setPassword('');
      setRetypedPassword('');
